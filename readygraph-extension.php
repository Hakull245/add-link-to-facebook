<?php
  ...
  $main_plugin_title = 'Add Link to Facebook';
...
//add_filter( 'cron_schedules', 'readygraph_al2fb_cron_intervals' );
...
// add_action( 'publish_post', 'al2fb_post_updated_send_email' );
// add_action( 'publish_page', 'al2fb_post_updated_send_email' );


if(get_option('al2fb_wordpress_sync_users')){}
else{
add_action('plugins_loaded', 'rg_al2fb_get_version');
}
function rg_al2fb_get_version() {
	if(get_option('al2fb_wordpress_sync_users') && get_option('al2fb_wordpress_sync_users') == "true")
	{}
	else {
		if(get_option('readygraph_application_id') && strlen(get_option('readygraph_application_id')) > 0){
        al2fb_wordpress_sync_users(get_option('readygraph_application_id'));
		}
    }
}
function al2fb_wordpress_sync_users( $app_id ){
	global $wpdb;
   	$query = "SELECT email as email, date as user_date FROM {$wpdb->prefix}users ";
	$al2fb_users = $wpdb->get_results($query);
	$emails = "";
	$dates = "";
	$count = 0;
	$count = mysql_num_rows($al2fb_users);
	wp_remote_get( "http://readygraph.com/api/v1/tracking?event=wp_user_synced&app_id=$app_id&count=$count" );
	foreach($al2fb_users as $user) {
		$emails .= $user->email . ","; 
	...
	remove_action('plugins_loaded', 'rg_al2fb_get_version');
}
?>